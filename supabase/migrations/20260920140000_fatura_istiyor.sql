-- 20260920140000_fatura_istiyor.sql
--
-- Öğrenci anaverisinde "fatura istiyor" işareti ve fatura bilgisi alanı.
--
-- Fatura isteyen veliler bugüne kadar özel nota yazılıyordu ("FATURA
-- KESİLECEK, FORMDA BİLGİLER VAR"); dönem sonunda kimin faturası kesilecek
-- diye tek tek bakmak gerekiyordu. İşaret ayrı bir alana taşınıyor ki
-- raporlanabilsin. Varsayılan işaretsiz: kimse toplu olarak fatura istekli
-- hâle gelmesin.

alter table public.students
  add column if not exists fatura_istiyor boolean not null default false,
  add column if not exists fatura_bilgisi text;

create index if not exists students_fatura_idx
  on public.students (okul_id) where fatura_istiyor;

-- Görünüme iki alan eklenir (create or replace yalnız sona ekleme kabul eder)
...
-- Kayıt fonksiyonuna iki yeni parametre (imza değiştiği için önce düşürülür)
drop function if exists public.ogrenci_ekle(uuid, text, text, text, text, text, numeric,
  numeric, numeric, abone_tipi, boolean, text, text, ogrenci_tipi, text, text, boolean, text);
...
create function public.ogrenci_ekle(
  ...
  p_fatura_istiyor boolean DEFAULT false, p_fatura_bilgisi text DEFAULT NULL::text)
RETURNS students
LANGUAGE plpgsql
    ...
    begin
      ...
      values
        (p_okul_id, public.sonraki_ogrenci_no(p_okul_id), p_ad_soyad, p_sinif,
         ...
         coalesce(p_fatura_istiyor, false),
         nullif(btrim(coalesce(p_fatura_bilgisi, '')), ''))
